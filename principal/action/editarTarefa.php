<?php
    ...
    if (!empty($_GET['id_tarefa'])) {
		...
		$codigo_sql = "SELECT * FROM $tabelaTarefa WHERE id_tarefa = $id_tarefa";
    ...
    // Verificando o click do botão, validando os INPUTs e realizando a atualização dos dados
...
<body class="bg-black">
    <div class="telaForm d-flex align-items-center justify-content-center">
        <form action="" method="POST" class="text-bg-dark shadow-lg rounded-2 position-relative form p-3 text-center d-flex align-items-center flex-column justify-content-around">
            <a href="../indexPrincipal.php" class="voltar btn text-light opacity-75 rounded-4 position-absolute">
            ...
            <div class="input-group mt-4 d-flex align-items-center">
                ...
                <input type="time" class="rounded me-3 p-1 input_time" name="tempoinicial" value="<?php echo $tempoInicio ?>"> -
                <input type="time" class="rounded ms-3 p-1 input_time" name="tempofinal" value="<?php echo $tempoFim ?>">
            </div>
            <div class="input-group mt-4 d-flex align-items-center">
